...
def get_data_physicalActivity(host_name, username, password):
    conn = psycopg2.connect(host = host_name, port = 5432, dbname = 'city4age', user = username, password = password)
    sql = '''
    WITH q0_gef AS (
        SELECT
        ...
        JOIN city4age_sr.time_interval AS ti ON gef.time_interval_id = ti. ID
        WHERE
            gef.gef_type_id IN (505, 839) -- walking or motility
        AND uir.pilot_code LIKE '%bhx%'
    ),
     q0_ges AS (
        SELECT
     ...
     q1_gef AS (
        ...
        WHERE
            uir.pilot_code LIKE '%bhx%' --  and dv.detection_variable_name = 'physical_activity'
        AND gef.gef_type_id = 505 -- motilitiy gef
        ORDER BY
            gef.user_in_role_id
    ),
     q1_ges AS (
        ...
        WHERE
            uir.pilot_code LIKE '%bhx%'
        AND gef.gef_type_id = 839 -- walking gef
        ORDER BY
            gef.user_in_role_id
    ),
     q1 AS (
    ...
    and q1.assessment_comment not like '%Slight reduction in walking steps%' -- redundant
    and q1.assessment_comment not like '%Both steps and distance plunged compared to September 2017%' -- only few measure points
    -- and q1.assessment_comment like '%Sharp drop%' and q1.assessment_comment like '%compared%'
    -- and q1.assessment_comment not like '%Sharp drop in both the number of steps and distance compared to previous month%'
    -- group BY
    -- 	q1.user_in_role_id,
    -- 	q1.interval_start,
...
def get_all_perc_changes():
    ...
    sql = '''
        ...
        WHERE
            --gef.gef_type_id IN (504, 514) -- walking or motility
            gef.gef_type_id IN (505) -- physical_activity
        AND uir.pilot_code LIKE '%bhx%'
    ),
     interm AS (
        SELECT
...
def prepare_and_save(data):
    ...
    pivoted_data = pd.pivot_table(data, index = ['user_in_role_id', 'interval_start'], columns='nui_name', values=['relative_change'], aggfunc=np.max)
    ...
    to_xlxs(pivoted_data, 'relative_changes')
...
def pivot_physicalActivity_data(data):
    motility_data = data[(data['gef_name']=='physical_activity') | (data['gef_name']=='physical activity')]
...
def get_motility_data(data):
    ...
    motility_data=motility_data.sort_values('risk_status')
    motility_data.reset_index(drop= True, inplace=True)
    to_xlxs(motility_data, name = 'assessments_0111_bi_09')
    # print (motility_data)
    return motility_data.iloc[:,3:]
...
def find_perc_chg():
    factor_data = pd.read_csv("Data//raw_assessments_1710_multi.csv")
    # motility_data.plot('')
    # gr_data = motility_data.groupby(['nui_name', 'risk_status'])['perc_change'].min()
    pivoted_data = pd.pivot_table(factor_data, index = ['nui_name', 'risk_status'], values='perc_change', aggfunc=[np.min, np.max, lam1, lam2, np.mean, np.std]).reset_index()
