...
from data_preparation_multi import get_data_multi, prepare_and_pivot_data, fill_missing_values, stack_dataset, normalize_dataset
...
def hmm_to_dict_multi_variate(data, prepared_data, model, dvId):
    ...
    stacked_data.set_index(['interval_start', 'detection_variable_name', 'normalised'], inplace= True)
    # print (stacked_data)

    clustered_data = data.set_index(['interval_start', 'detection_variable_name', 'normalised']).join(stacked_data)
    ...
    for i in range(0, model.n_components):  # iterate through clusters
        ...
        if (len(np.unique(mask)) == 2):
            ...
            clusterData.append(cluster[i])
        # cluster[i].update({'name' : name, 'items' : replaceNullValues(cluster_points)})
    dict.update({'cluster': clusterData, 'mean': means_arranged, 'std': vars_arranged,
                 'trans_mat': trans_mat_arranged, 'groups': time_intervals.tolist(), 'vmvid': vmv_ids.tolist()})
    # print (dict)
    return dict
...
def start_multi(userId, dvId):
    ...
    pivoted_data = prepare_and_pivot_data(data)
    prepared_dataset = fill_missing_values(pivoted_data)
...
start_multi(userId=107, dvId=74)
