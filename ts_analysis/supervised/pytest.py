...
data = np.array([['', 'col1', 'col2'],
                ...
                ['row2', 3, 4]])
# print (data[1:,0])
df = pd.DataFrame(data=data[1:,1:].astype(int), index=data[0,1:],columns=data[0,1:])
print(df)
...
filters=np.array([['', 'col1', 'col2', 'col3'],
                 ...
                 ['row3', 2, 2, 'a'],
                  ['row4', 2, 4, 'a']])
dff = pd.DataFrame(data=filters[1:,1:], index=filters[0,1:],columns=filters[0,1:])
dff[['col1', 'col2']].apply(pd.to_numeric, errors='coerce')
print (dff)

rdf = df.join(dff, how='left', lsuffix='_1', rsuffix='_2', on=['col1', 'col2'])
print (rdf)
